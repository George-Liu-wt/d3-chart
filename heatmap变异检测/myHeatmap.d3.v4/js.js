jQuery.BSA_linechart = {
  show_BSA_linechart:function(container,contents)
  {
    ...
    var margin = { top: 100, right: 80, bottom: 80, left: 65 },
        ...
        height = contents.datas.data1.length * contents.size.height;
    ...
    for (i = 0; i < contents.datas.data1.length; i++) {
      var value_list = contents.datas.data1[i];
    ...
    var chr_amount = contents.datas.data1.length;

    var gridSizeX = ((width - margin.right) / maxlistlength); /*格子大小的参数*/
    var gridSizeY = Math.floor((height - contents.datas.data1.length * 10) / chr_amount / 1 ); /*格子大小的参数*/
    ...
    list_datas = [];
    for (i = 0; i < contents.datas.data1.length; i++) {
      list_datas = list_datas.concat(contents.datas.data1[i])
    }

    for (i = 0; i < contents.datas.data1.length; i++) {
    var cards = g.selectAll(".xamounts")
        .data(contents.datas.data1[i]);
   ...
   cards.enter()   /*每个格子。*/
      ...
      /*在svg中添加g元素并绘制。*/
       ...
       g.append("g")
                   ...
                   .attr("transform","translate("+ ((width -margin.right)/ 2 - 18 * contents.params.title.length / 2) +",-60)")
