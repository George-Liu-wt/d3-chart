...
var graph = {
   ...
   cards.enter()   //每个格子。
        ...
        .on('mouseout', function() {
          clearTimeout(timeout);
          d3.select('.distribution_tooltip').style('opacity', 0);});
    ...
    // console.log(contents.params.Legend_colors.length);
    // console.log(colors);
    ...
    console.log(contents.params.marker_colors.length);
    for (i = 0; i < data.length; i++) {
         var g = svg.append("g")
                    .attr("transform","translate("+ margin.left +","+ margin.top +")");
