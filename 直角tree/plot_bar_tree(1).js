/* globals $: true, jQuery: true, d3: true */
        ...
        for (var i = 0; i < tokens.length; i++) {
            ...
            default:
                ...
                if (x == ')' || x == '(' || x == ',') {
                    var x2 = tokens[i + 1];
                    if (x2 == ':' && x == ')'){
                        tree.name = tokens[i+1];
                        tree.percent = parseFloat(token);
                    }else{
                        tree.name = token;
                    }
                } else if (x == ':') {
                    tree.length = parseFloat(token);
                    tree.weight = parseFloat(token);
                }
            }
        }
        return tree;
...
function plotTree(div, content) {
    ...
    var legendBarData = [];
    var show_bootstrap = (typeof(content.params.show_bootstrap) != 'undefined' && content.params.show_bootstrap == 'true') ? true : false;
    for (var g in groupBar) {
        legendBarData.push({
            "name": g,
            "color": groupBar[g]
        });
    ...
    var title_g = svgContainer.append("g").attr("class", "title").attr("transform", "translate(" + (svg_w / 2 -6.4*title.length) + ", " + margin_top / 3 + ")");
    ...
    function plot_one(tree) {
        ...
        if (tree.branchset) {
            ...
            if (show_bootstrap == true && tree.percent != undefined && !isNaN(tree.percent) &&tree.percent>=0.5) {
                tree_g.append("text").text(parseInt(Math.floor(tree.percent * 10000) / 100)).attr("font-family", 'arial').attr("font-size", "10px").attr("transform", "translate(" +  linear_width(tree.start_x) + ", " + linear_height(tree.y) + ")");
            }
            ...
            tree_g.append("line").attr("x1", linear_width(tree.end_x)).attr("y1", linear_height(tree.y)).attr("x2", maxLength - tree.name.length * 6).attr("y2", linear_height(tree.y)).attr("stroke-width", 0.3).attr("stroke-dasharray", 2).attr("stroke", '#008000');
        }
    }
    ...
    }).style("font-size", '10px').attr("fill", function(d) {
            if(typeof(content.select_leaf)!='' && typeof(content.select_leaf)!='undefined' && d.name ==content.select_leaf){
                return content.select_leaf_color?content.select_leaf_color:"#FF0000";
            }
        });
...
function addBar(barData, groupColor, barChartG, width, height, treeWidth, bottomH, margin_top, barHeightO, ifChartText) {
    ...
    if(ifChartText==="true"){
        var barTitle = barChartG.append("text").text("Number of reads").attr("font-family", "arial").attr("font-size", "12px").attr("text-anchor", "right").attr("transform", "translate(" + barWidth / 2 + ", -20)");
    };
